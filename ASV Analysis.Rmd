...
#Now average every OTU relabund value from relabund.t.fish.F4.F8.ST.cull for each of the species.
relabund.t.fish.F4.F8.ST.cull=cbind(relabund.t.fish.F4.F8.ST.cull,metadata.fish.F4.F8.noPC.unifrac) #first merge metadata
relabund.t.fish.F4.F8.ST.cull.species.mean=as.data.frame(matrix(nrow=3,ncol=749)) #create a blank df to store averaged outputs
for (i in 1:749){ #for each OTU column
  relabund.t.fish.F4.F8.ST.cull.species.mean[,i]=aggregate(relabund.t.fish.F4.F8.ST.cull[,i],by=list(relabund.t.fish.F4.F8.ST.cull$Species),FUN=mean)[[2]] #aggregate by gut subsection, calculate means, and store means in bank df.
}
colnames(relabund.t.fish.F4.F8.ST.cull.species.mean)=colnames(relabund.t.fish.F4.F8.ST.cull)[1:749] #add in OTU names
rownames(relabund.t.fish.F4.F8.ST.cull.species.mean)=aggregate(relabund.t.fish.F4.F8.ST.cull[,i],by=list(relabund.t.fish.F4.F8.ST.cull$Species),FUN=mean)[[1]] #add in Species as rownames
...
metadata.fish.F4.F8.noPC.unifrac.species.mean=as.data.frame(matrix(nrow=3,ncol=1))
colnames(metadata.fish.F4.F8.noPC.unifrac.species.mean)="Species"
rownames(metadata.fish.F4.F8.noPC.unifrac.species.mean)=rownames(relabund.t.fish.F4.F8.ST.cull.species.mean)
metadata.fish.F4.F8.noPC.unifrac.species.mean$Species=rownames(relabund.t.fish.F4.F8.ST.cull.species.mean)
...
#Now do the same for the species averaged data
#Convert relabund data, taxonomy data, and metadata to phyloseq objects and merge them. Use the F4.F8 PC culled data, averaged across gut subsections.
