...
ordiellipse(nmds.relabund.2,metadata.19.jun.samples2$Gut_Section,kind="sd",draw="polygon",lwd=2.5,border=c("mediumpurple1",viridis(n=3,option="D")[2:3]),col=c("mediumpurple1",viridis(n=3,option="D")[2:3])) #add SD ellipses, colored by Gut_Section.
...
#write.csv(permanova.gut.subsection$aov.tab,file="permanova.gut.subsection.csv")
...
generate.long.format(mixed.mod.sig.feature.Gut_SubSection.1,metadata.19.jun.samples3,sig.feature.subsection.data) #generate longformat
...
abund.longformat3=merge(abund.longformat2,sig.feature.subsection.data[,5:28],by.x="row_ID",by.y="OTUNumber",all.x=T,all.)
...
ggplot(abund.longformat3[abund.longformat3$Library.Common.Name!="N/A",],aes(x=Gut_SubSection,y=mean_relabund,color=Library.Common.Name,group=row_ID))+
  ...
  geom_pointrange(aes(ymin=mean_relabund-SE,ymax=mean_relabund+SE))+
    facet_wrap(~Component.Index,scales="free")+
    theme(legend.position="bottom")
#export as 2500x1250 PNG and save as mixed.mod.significant.gut.subsection.features2
