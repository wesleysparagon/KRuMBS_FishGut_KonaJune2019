...
#make a histogram of the feature peak area values EXCLUDING 0s. The data is very 0 inflated.
...
#The highest relabund freq is very low, between .0004 and .0008. The data has a strong right skew.
```
...
bray.relabund.19.jun.clean2.2=vegdist(relabund.19.jun.clean2.dist.t.2,method="bray")

#reorder the metadata so it lines up with the distance matrix
metadata.19.jun.samples2=metadata.19.jun.samples2[match(rownames(relabund.19.jun.clean2.dist.t.2),metadata.19.jun.samples2$sample_name2),]
...
legend(-.75,-.045,legend = c("ST","GI_1","GI_2","GI_3","GI_4","HG_1","HG_2","HG_3","K. cinerascens","K. hawaiiensis","K. vaigiensis"),cex=1,y.intersp=.75,bg="transparent",bty="n",col=c(colvec3,"black","black","black"),pch=c(16,16,16,16,16,16,16,16,15,16,17),pt.cex=.75)
```
...
#re order the matrix so samples are in the same order as the metabolomic dist matrix
weighted.unifrac.F4.F8.matrix1=weighted.unifrac.F4.F8.matrix1[match(metadata.19.jun.samples2$SampleCode,rownames(weighted.unifrac.F4.F8.matrix1)),match(metadata.19.jun.samples2$SampleCode,rownames(weighted.unifrac.F4.F8.matrix1))]
...
Interestingly, it appears as though the mantel test is not significant? Next, run procrustes.
...
plot(pro,kind=1,lwd=1.5,len=.06)
