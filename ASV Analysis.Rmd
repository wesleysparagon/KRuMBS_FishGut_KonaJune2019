...
metadata.fish.F4.F8.noPC.unifrac$Gut_Section=factor(metadata.fish.F4.F8.noPC.unifrac$Gut_Section,levels=c("Foregut","Midgut","Hindgut","Stomach")) #reorder the levels of the Gut_Section factor. This will be useful later when making plots.
...
relabund.fish.F4.F8.ST=relabund[,colnames(relabund) %in% metadata.fish.F4.F8.noPC.unifrac$SampleCode]
relabund.fish.ST=relabund[,colnames(relabund) %in% metadata.fish.noPC.unifrac$SampleCode]
...
relabund.t.fish.F4.F8.ST=relabund.t[rownames(relabund.t) %in% metadata.fish.F4.F8.noPC.unifrac$SampleCode,]
...
relabund.fish.F4.F8.ST.zerocull=relabund.fish.F4.F8.ST[apply(relabund.fish.F4.F8.ST[,1:39],1,FUN=sum)!=0,]

taxonomy.fish=relabund.fish[,42:48] #make a taxonomy df for fish ASVs
taxonomy.fish.F4.F8.ST=relabund.fish.F4.F8.ST[,42:48]
```
...
relabund.t.fish.F4.F8.ST.cull=cbind(relabund.t.fish.F4.F8.ST.cull,metadata.fish.F4.F8.noPC.unifrac) #first merge metadata
relabund.t.fish.F4.F8.ST.cull.mean=as.data.frame(matrix(nrow=8,ncol=749)) #create a blank df to store averaged outputs
for (i in 1:749){ #for each OTU column
...
colnames(relabund.t.fish.F4.F8.ST.cull.mean)=colnames(relabund.t.fish.F4.F8.ST.cull)[1:749] #add in OTU names
...
#Now average every OTU relabund value from relabund.t.fish.F4.F8.ST.cull for each of the species and gut sections.
relabund.t.fish.F4.F8.ST.cull$species.section=paste(relabund.t.fish.F4.F8.ST.cull$Species,relabund.t.fish.F4.F8.ST.cull$Gut_Section,sep="_") #create a new factor that merges species and section. this is what I will average across
relabund.t.fish.F4.F8.ST.cull.species.mean=as.data.frame(matrix(nrow=9,ncol=749)) #create a blank df to store averaged outputs
for (i in 1:749){ #for each OTU column
  relabund.t.fish.F4.F8.ST.cull.species.mean[,i]=aggregate(relabund.t.fish.F4.F8.ST.cull[,i],by=list(relabund.t.fish.F4.F8.ST.cull$species.section),FUN=mean)[[2]] #aggregate by gut subsection, calculate means, and store means in bank df.
}
colnames(relabund.t.fish.F4.F8.ST.cull.species.mean)=colnames(relabund.t.fish.F4.F8.ST.cull)[1:749] #add in OTU names
rownames(relabund.t.fish.F4.F8.ST.cull.species.mean)=aggregate(relabund.t.fish.F4.F8.ST.cull[,i],by=list(relabund.t.fish.F4.F8.ST.cull$species.section),FUN=mean)[[1]] #add in Species as rownames

#create a metadata df for relabund.t.fish.F4.F8.ST.cull.mean that can be used in phyloseq.
metadata.fish.F4.F8.noPC.unifrac.mean=as.data.frame(matrix(nrow=8,ncol=1))
colnames(metadata.fish.F4.F8.noPC.unifrac.mean)="Gut_Subsection"
rownames(metadata.fish.F4.F8.noPC.unifrac.mean)=rownames(relabund.t.fish.F4.F8.ST.cull.mean)
metadata.fish.F4.F8.noPC.unifrac.mean$Gut_Subsection=rownames(relabund.t.fish.F4.F8.ST.cull.mean)

#create a metadata df for relabund.t.fish.F4.F8.ST.cull.species.mean that can be used in phyloseq.
metadata.fish.F4.F8.noPC.unifrac.species.mean=as.data.frame(matrix(nrow=9,ncol=3))
colnames(metadata.fish.F4.F8.noPC.unifrac.species.mean)=c("Species","Gut_Section","Species_Gut_Section")
rownames(metadata.fish.F4.F8.noPC.unifrac.species.mean)=rownames(relabund.t.fish.F4.F8.ST.cull.species.mean)
metadata.fish.F4.F8.noPC.unifrac.species.mean$Species=c("Kyphosus cinerascens","Kyphosus cinerascens","Kyphosus cinerascens","Kyphosus hawaiiensis","Kyphosus hawaiiensis","Kyphosus hawaiiensis","Kyphosus vaigiensis","Kyphosus vaigiensis","Kyphosus vaigiensis")
metadata.fish.F4.F8.noPC.unifrac.species.mean$Gut_Section=c("GI","HG","ST","GI","HG","ST","GI","HG","ST")
metadata.fish.F4.F8.noPC.unifrac.species.mean$Species_Gut_Section=rownames(metadata.fish.F4.F8.noPC.unifrac.species.mean)
...
metadata.phyloseq=sample_data(metadata.fish.F4.F8.noPC.unifrac.mean)
...
other.otus1=other.otus1[,c(7,1:6)]
...
write.csv(relabund.family.df,file="relabund.family.df.csv") #Export the ralbund.cull.genus (or family) data as a CSV and open it in JMP. Use JMP to create stacked bar charts.
...
taxonomy.phyloseq=tax_table(as.matrix(taxonomy.fish.F4.F8.ST.cull))
metadata.phyloseq.species=sample_data(metadata.fish.F4.F8.noPC.unifrac.species.mean)
