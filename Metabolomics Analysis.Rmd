...
plot(nmds.relabund.2,type="n",ylim=c(-.35,.6),xlim-c(-.7,.7))
...
cull.otu(relabund.19.jun.clean2.dist.t.3,3,.0005,.01)
relabund.19.jun.clean2.dist.t.3.cull=relabund.df.cull
#went from 1133 ASVs to 583 ASVs

#asinsqrt transform the data
relabund.19.jun.clean2.dist.t.3.cull.trans=asin(sqrt(relabund.19.jun.clean2.dist.t.3.cull))
#merge in metadata
relabund.19.jun.clean2.dist.t.3.cull.trans=cbind(relabund.19.jun.clean2.dist.t.3.cull.trans,metadata.19.jun.samples2)

#remove stomach samples
#relabund.19.jun.clean2.dist.t.3.cull.trans1=relabund.19.jun.clean2.dist.t.3.cull.trans[relabund.19.jun.clean2.dist.t.3.cull.trans$Gut_SubSection!="ST",]
...
mixed.mod=function(x) { #create a function that performs the desired mixed model
  mod=lmer(x~Species+Gut_SubSection+(1|Fish_Number),data=relabund.19.jun.clean2.dist.t.3.cull.trans)
  return(mod)
}
mixed.mod.relabund.19.jun.clean2.dist.t.3.cull.trans=apply(relabund.19.jun.clean2.dist.t.3.cull.trans[,-584:-602],2,FUN=mixed.mod) #apply mixed.mod only to columns containing ASV data. Save as new object.

#Now let's inspect the results of the mixed models.
mixed.mod.singularity=lapply(mixed.mod.relabund.19.jun.clean2.dist.t.3.cull.trans,FUN=isSingular) #test each mixed model for singularity, save in list. After inspecting the list it looks like ~50% of models are singular.
...
mixed.mod.results=lapply(mixed.mod.relabund.19.jun.clean2.dist.t.3.cull.trans,FUN=anova.kw) #perform an anova(ddf="Kenward-Roger") on all mixed models, save output.
mixed.mod.pvals=as.data.frame(t(as.data.frame(mixed.mod.results))[seq(from=6,to=3498,by=6),]) #extract the pvals from the mixed.mod.results list. Do this by converting list to df, transposing it, and then extracting every 6th column (which correspond to the pvals from each model).
...
rownames(mixed.mod.pvals)=colnames(relabund.19.jun.clean2.dist.t.3.cull.trans)[1:583] #Change rownames to correspond with tested OTU names.
mixed.mod.sig.feature.Gut_SubSection=mixed.mod.pvals[mixed.mod.pvals$Gut_SubSection.padjust<=.05,] #subset the mixed.mod.pvals df for significant (p<=.05) Gut_SubSection pvals. Save as new df.
...
molnetdata1=molnetdata1[match(rownames(mixed.mod.sig.feature.Gut_SubSection),molnetdata1$cluster.index),]


mixed.mod.sig.feature.Gut_SubSection=cbind(mixed.mod.sig.feature.Gut_SubSection,molnetdata1) #merge mixed.mod.sig.otu.Gut_SubSection with the associated OTU taxonomies.
