...
ordiellipse(nmds.weighted.unifrac.F4.F8,metadata.fish.F4.F8.unifrac$Gut_Section,kind="sd",draw="polygon",alpha=c(0,0),lwd=2.5,border=colvec3) #add SD ellipses, colored by Gut_Section. 
text(-.5,.3,label="Stress=.14")
#Export as 8.5x11 PDF
```
...
```{r}
colvec2=viridis(n=8,option="D")
plot(nmds.weighted.unifrac.F4.F8,type="n",xlim=c(-.5,.53),ylim=c(-.4,.4)) #Set up the NMDS plot environment
points(nmds.weighted.unifrac.F4.F8, #plot each sample as a point
       cex=2,
       pch=pointvec[metadata.fish.F4.F8.unifrac$Species], #point shape corresponds to fish Species
       col=colvec2[as.factor(metadata.fish.F4.F8.unifrac$Gut_SubSection)]) 
...
hist(alpha.diversity.fish$sobs) #asses normality of sobs data
shapiro.test(alpha.diversity.fish$sobs) 
```

The sobs data appearto be normal.

```{r}
hist(alpha.diversity.fish$shannoneven) #asses normality of shannoneven data
shapiro.test(alpha.diversity.fish$shannoneven)
...
```{r}
#use the GI culled data!
#Model the sobs data in response to Gut_seciton, fish species, and fish ID
adiv.mod.gut.nested.sobs=lmer(sobs~Gut_Section/Gut_SubSection+Species+(1|Fish_Number),data=alpha.diversity.fish)
...
lsmeans(adiv.mod.gut.nested.sobs, pairwise ~ Gut_Section, adjust="tukey") #HG and ST are not significantly different from each other, both are significantly different from GI.

#Model the shannoneven data in response to Gut_seciton, fish species, and fish ID
adiv.mod.gut.section.shannoneven=lmer(shannoneven~Gut_Section+Species+(1|Fish_Number),data=alpha.diversity.fish)
...
#plot_grid(sobs.gut.section.plot,shannoneven.gut.section.plot) #use the plot_grid function to plot the above 4 plots (par(mfrow=())) doesnt work with ggplot2. Export as PNG (800 x 455). This line now produces the error "Error: Aesthetics must be either length 1 or the same as the data (5): x, y and label". Since I am not using these graphs I am not going to deal with it right now.
```
...
#Repeat these models with Species and Fish_ID nested as randome effects. Model the sobs data in response to Gut_SubSeciton, fish species, and fish ID
adiv.mod.gut.subsection.sobs1=lmer(sobs~Gut_SubSection+(1|Species/Fish_Number),data=alpha.diversity.fish.F4.F8) #interaction term has been removed to prevent errors.
summary(adiv.mod.gut.subsection.sobs1)
anova(adiv.mod.gut.subsection.sobs1,ddf="Kenward-Roger") #Gut_SubSection is significant, Species is not significant.
lsmeans(adiv.mod.gut.subsection.sobs1, pairwise ~ Gut_SubSection, adjust="tukey") #Post Hoc analysis for the Gut_SubSection factor. This function averages across Species so that it is ONLY testing the differences between Gut_SubSection. 

#Model the shannoneven data in response to Gut_subseciton, fish species, and fish ID
adiv.mod.gut.subsection.shannoneven1=lmer(shannoneven~Gut_SubSection+(1|Species/Fish_Number),data=alpha.diversity.fish.F4.F8)
summary(adiv.mod.gut.subsection.shannoneven1)
anova(adiv.mod.gut.subsection.shannoneven1,ddf="Kenward-Roger") #Gut_SubSection is significant, species is not.
lsmeans(adiv.mod.gut.subsection.shannoneven1, pairwise ~ Gut_SubSection, adjust="tukey")
```
...
dispersion.mod.gut.subsection=lmer(sqrt.distances~Gut_SubSection+(1|Fish_Number),data=dispersion.gut.subsection1)
...
dispersion.gut.subsection.plot.v1=ggplot(dispersion.gut.subsection2,aes(y=distance_to_centroid,x=Gut_SubSection,fill=Gut_SubSection))+
  geom_bar(stat="identity")+
