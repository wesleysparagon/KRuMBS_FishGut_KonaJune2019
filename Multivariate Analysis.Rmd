...
Both Gut Section/SubSection and Species have a significant effect on fish gut microbiome community structure, with no significant interaction. When Species and Gut_Section are the predictors, Gut_Section has a stronger effect on the fish gut microbiome then species. When Species and Gut_SubSection are the predictors, they have similar strength effects on the fish gut microbiome (Species has a slightly higher F value).
...
colvec=viridis(n=4,option="D") #generate a vector of colors to correspond to Gut_Section.
...
#This plot is the same as the above ggplot, but also has ellipses and no axis
colvec2=viridis(n=9,option="D")
plot(nmds.weighted.unifrac.F4.F8,type="n",xlim=c(-.37,.68)) #Set up the NMDS plot environment
points(nmds.weighted.unifrac.F4.F8, #plot each sample as a point
       cex=2,
       pch=pointvec[metadata.fish.F4.F8.1$Species], #point shape corresponds to fish Species
       col=colvec2[as.factor(metadata.fish.F4.F8.1$Ranked_Gut_SubSection_Distance_from_Stomach)]) 
ordiellipse(nmds.weighted.unifrac.F4.F8,metadata.fish.F4.F8.1$Ranked_Gut_SubSection_Distance_from_Stomach,show.groups=c("0","1","2","3","4","5","6","7","8"),kind="sd",draw="polygon",alpha=c(0,0),lwd=2.5,border=colvec2[1:9])

legend(.4,.43,legend = c("ST","PC","GI_1","GI_2","GI_3","GI_4","HG_1","HG_2","HG_3","K. cinerascens","K. hawaiiensis","K. vaigiensis"),cex=1,y.intersp=.3,bg="transparent",bty="o",col=c(colvec2,"black","black","black"),pch=c(16,16,16,16,16,16,16,16,16,15,16,17),pt.cex=1.5) #Add legend. Save image as 8.5x11 PDF (landscape)
...
points(nmds.weighted.unifrac.F4.F8, #plot each sample as a point
       ...
       pch=pointvec[metadata.fish.F4.F8.1$Species], #point shape corresponds to fish Species
       col=colvec2[as.factor(metadata.fish.F4.F8.1$Ranked_Gut_SubSection_Distance_from_Stomach)]) 
ordiarrows(nmds.weighted.unifrac.F4.F8,group=metadata.fish.F4.F8.1$Fish_Number,order.by=metadata.fish.F4.F8.1$Ranked_Gut_SubSection_Distance_from_Stomach,show.groups=c("F7","F8"),lwd=2,col="black")

legend(.4,.45,legend = c("ST","PC","GI_1","GI_2","GI_3","GI_4","HG_1","HG_2","HG_3","K. cinerascens","K. hawaiiensis","K. vaigiensis"),cex=1,y.intersp=.3,bg="transparent",bty="o",col=c(colvec2,"black","black","black"),pch=c(16,16,16,16,16,16,16,16,16,15,16,17),pt.cex=1.5) #Add legend. Save image as 8.5x11 PDF (landscape)
...
#Calculate means and SE of the various alpha diversity metrics for each Gut_Section and save as a df
alpha.diversity.mean.gut_section=aggregate(alpha.diversity.fish$sobs,by=list(alpha.diversity.fish$Gut_Section),FUN=mean)$x #aggregate (mean) sobs by Gut_Section, save in new df.
alpha.diversity.mean.gut_section=as.data.frame(alpha.diversity.mean.gut_section)
colnames(alpha.diversity.mean.gut_section)[1]="sobs" #rename column 1 to sobs.
alpha.diversity.mean.gut_section$SE_sobs=aggregate(alpha.diversity.fish$sobs,by=list(alpha.diversity.fish$Gut_Section),FUN=stderror)$x #aggregate (stderror) sobs by Gut_section, save as a new column in df.
#repeat this process for chao
alpha.diversity.mean.gut_section$chao=aggregate(alpha.diversity.fish$chao,by=list(alpha.diversity.fish$Gut_Section),FUN=mean)$x
alpha.diversity.mean.gut_section$SE_chao=aggregate(alpha.diversity.fish$chao,by=list(alpha.diversity.fish$Gut_Section),FUN=stderror)$x
#repeat this process for shannon
alpha.diversity.mean.gut_section$shannon=aggregate(alpha.diversity.fish$shannon,by=list(alpha.diversity.fish$Gut_Section),FUN=mean)$x
alpha.diversity.mean.gut_section$SE_shannon=aggregate(alpha.diversity.fish$shannon,by=list(alpha.diversity.fish$Gut_Section),FUN=stderror)$x
#repeat this for shannoneven
alpha.diversity.mean.gut_section$shannoneven=aggregate(alpha.diversity.fish$shannoneven,by=list(alpha.diversity.fish$Gut_Section),FUN=mean)$x
alpha.diversity.mean.gut_section$SE_shannoneven=aggregate(alpha.diversity.fish$shannoneven,by=list(alpha.diversity.fish$Gut_Section),FUN=stderror)$x
#now add in a Gut_Section column
alpha.diversity.mean.gut_section$Gut_Section=aggregate(alpha.diversity.fish$shannoneven,by=list(alpha.diversity.fish$Gut_Section),FUN=stderror)$Group.1

#Now graph all 4 alpha diversity indices as a function of Gut_Section
sobs.gut.section.plot=ggplot(alpha.diversity.mean.gut_section,aes(x=Gut_Section,y=sobs,fill=Gut_Section))+
  ...
  theme_classic()
#now repeat for chao
chao.gut.section.plot=ggplot(alpha.diversity.mean.gut_section,aes(x=Gut_Section,y=chao,fill=Gut_Section))+
  geom_bar(stat="identity")+
  scale_fill_manual(values=viridis(n=4))+
...
shannon.gut.section.plot=ggplot(alpha.diversity.mean.gut_section,aes(x=Gut_Section,y=shannon,fill=Gut_Section))+
  geom_bar(stat="identity")+
  scale_fill_manual(values=viridis(n=4))+
...
shannoneven.gut.section.plot=ggplot(alpha.diversity.mean.gut_section,aes(x=Gut_Section,y=shannoneven,fill=Gut_Section))+
  geom_bar(stat="identity")+
  scale_fill_manual(values=viridis(n=4))+
...
plot_grid(sobs.gut.section.plot,chao.gut.section.plot,shannon.gut.section.plot,shannoneven.gut.section.plot) #use the plot_grid function to plot the above 4 plots (par(mfrow=())) doesnt work with ggplot2. Export as PNG (800 x 455).
```

Sobs and Chao show both ST and HG to have substantially higher alpha diversity compared to PC and GI. Shannon and shannon eveness show HG to have substantially higher alpha diversity than all other gut sections.

Next, plot the same alpha diversity metrics by Species.

```{r}
#Calculate means and SE of the various alpha diversity metrics for each Species and save as a df
alpha.diversity.mean.Species=aggregate(alpha.diversity.fish$sobs,by=list(alpha.diversity.fish$Species),FUN=mean)$x #aggregate (mean) sobs by Species, save in new df.
alpha.diversity.mean.Species=as.data.frame(alpha.diversity.mean.Species)
colnames(alpha.diversity.mean.Species)[1]="sobs" #rename column 1 to sobs.
alpha.diversity.mean.Species$SE_sobs=aggregate(alpha.diversity.fish$sobs,by=list(alpha.diversity.fish$Species),FUN=stderror)$x #aggregate (stderror) sobs by Species, save as a new column in df.
#repeat this process for chao
alpha.diversity.mean.Species$chao=aggregate(alpha.diversity.fish$chao,by=list(alpha.diversity.fish$Species),FUN=mean)$x
alpha.diversity.mean.Species$SE_chao=aggregate(alpha.diversity.fish$chao,by=list(alpha.diversity.fish$Species),FUN=stderror)$x
#repeat this process for shannon
alpha.diversity.mean.Species$shannon=aggregate(alpha.diversity.fish$shannon,by=list(alpha.diversity.fish$Species),FUN=mean)$x
alpha.diversity.mean.Species$SE_shannon=aggregate(alpha.diversity.fish$shannon,by=list(alpha.diversity.fish$Species),FUN=stderror)$x
#repeat this for shannoneven
alpha.diversity.mean.Species$shannoneven=aggregate(alpha.diversity.fish$shannoneven,by=list(alpha.diversity.fish$Species),FUN=mean)$x
alpha.diversity.mean.Species$SE_shannoneven=aggregate(alpha.diversity.fish$shannoneven,by=list(alpha.diversity.fish$Species),FUN=stderror)$x
#now add in a Gut_Section column
alpha.diversity.mean.Species$Species=aggregate(alpha.diversity.fish$shannoneven,by=list(alpha.diversity.fish$Species),FUN=stderror)$Group.1

#Now graph all 4 alpha diversity indices as a function of Species
sobs.Species.plot=ggplot(alpha.diversity.mean.Species,aes(x=Species,y=sobs,fill=Species))+
  geom_bar(stat="identity")+
  scale_fill_manual(values=viridis(n=4))+
  geom_errorbar(aes(ymin=sobs-SE_sobs,ymax=sobs+SE_sobs),width=.5)+
  ggtitle("Observed Sequences")+
  theme_classic()
#now repeat for chao
chao.Species.plot=ggplot(alpha.diversity.mean.Species,aes(x=Species,y=chao,fill=Species))+
  geom_bar(stat="identity")+
  scale_fill_manual(values=viridis(n=4))+
  geom_errorbar(aes(ymin=chao-SE_chao,ymax=chao+SE_chao),width=.5)+
  ggtitle("Chao")+
  theme_classic()
#now repeat for shannon
shannon.Species.plot=ggplot(alpha.diversity.mean.Species,aes(x=Species,y=shannon,fill=Species))+
  geom_bar(stat="identity")+
  scale_fill_manual(values=viridis(n=4))+
  geom_errorbar(aes(ymin=shannon-SE_shannon,ymax=shannon+SE_shannon),width=.5)+
  ggtitle("Shannon")+
  theme_classic()
#now repeat for shannoneven
shannoneven.Species.plot=ggplot(alpha.diversity.mean.Species,aes(x=Species,y=shannoneven,fill=Species))+
  geom_bar(stat="identity")+
  scale_fill_manual(values=viridis(n=4))+
  geom_errorbar(aes(ymin=shannoneven-SE_shannoneven,ymax=shannoneven+SE_shannoneven),width=.5)+
  ggtitle("Shannon Eveness")+
...
plot_grid(sobs.Species.plot,chao.Species.plot,shannon.Species.plot,shannoneven.Species.plot) #use the plot_grid function to plot the above 4 plots (par(mfrow=())) doesnt work with ggplot2.

```

There appears to be no clear difference in any of the alpha diversity metrics between species.

Next, plot the same alpha diversity metrics by Gut_SUBsection.

```{r}
#Calculate means and SE of the various alpha diversity metrics for each Gut_SubSection and save as a df
...
#repeat this process for chao
alpha.diversity.mean.Gut_SubSection$chao=aggregate(alpha.diversity.fish.F4.F8$chao,by=list(alpha.diversity.fish.F4.F8$Gut_SubSection),FUN=mean)$x
alpha.diversity.mean.Gut_SubSection$SE_chao=aggregate(alpha.diversity.fish.F4.F8$chao,by=list(alpha.diversity.fish.F4.F8$Gut_SubSection),FUN=stderror)$x
#repeat this process for shannon
...
#Now graph all 4 alpha diversity indices as a function of Gut_SubSection
