...
sig.feature.subsection=read.csv(file="sig.feature.subsection.data_ECG_v1.csv")
...
```{r}
permanova.gut.section=adonis2(relabund.19.jun.clean2.dist.t~Gut_Section+Species,by="margin",data=metadata.19.jun.samples1)
...
metadata.19.jun.samples2$Gut_SubSection1=metadata.19.jun.samples2$Gut_SubSection #create a duplicate gut subsection col
metadata.19.jun.samples2$Gut_SubSection1[metadata.19.jun.samples2$Gut_Section=="HG"]="HG"
...
ordiellipse(nmds.relabund.2,metadata.19.jun.samples2$Gut_SubSection1,kind="sd",draw="polygon",lwd=2.5,border=colvec3[-6:-7],col=colvec3[-6:-7]) #add SD ellipses, colored by Gut_Section.
...
#export as 8.5x9 PDF and use in Figure X3

c("mediumpurple1",viridis(n=3,option="D")[2:3])
```
...
mixed.mod.section.relabund.19.jun.clean2.dist.t.4.cull.trans=apply(relabund.19.jun.clean2.dist.t.4.cull.trans[,-550:-569],2,FUN=mixed.mod.section) #apply mixed.mod only to columns containing ASV data. Save as new object.
...
rownames(mixed.mod.section.pvals)=colnames(relabund.19.jun.clean2.dist.t.4.cull.trans)[1:549] #Change rownames to correspond with tested feature names.
...
#repeat for mixed.mod.section
libraryID1=data[data$row.ID %in% rownames(mixed.mod.section.sig.feature.Gut_Section),1:8]
...
```{r}
#plot all the features together
ggplot(abund.longformat1,aes(x=Gut_SubSection.y,y=abund,color=Library.Common.Name,group=row_ID))+
  geom_point()+geom_smooth()+
    facet_wrap(~Component.Index,scales="free")+
    theme(legend.position="bottom")
#looks surprsingly good. Now try with just smoother.
...
#now plot avergae relabund for each features across gut subsections, colored by lib ID and faceted out by subnetwork.
#ggplot(abund.longformat3,aes(x=Gut_SubSection,y=mean_relabund,color=Library.Common.Name,group=row_ID))+
  #geom_point(size=3)+geom_line(size=1.5)+
    #facet_wrap(~Component.Index,scales="free")+
   # theme(legend.position="bottom")
#there are a few networks that look bad. specifically: 15 and 45

#now try the same plot without NA lib IDs and geom_bar
#ggplot(abund.longformat3[abund.longformat3$Library.Common.Name!="N/A",],aes(x=Gut_SubSection,y=mean_relabund,fill=Library.Common.Name,group=row_ID))+
  #geom_bar(stat="identity",col="black")+
    #facet_wrap(~Component.Index,scales="free")+
    #theme(legend.position="bottom")
#export as 2500x1500 PNG and save as mixed.mod.significant.gut.subsection.features

#now try the plot points and lines
#ggplot(abund.longformat3[abund.longformat3$Library.Common.Name!="N/A",],aes(x=Gut_SubSection,y=mean_relabund,color=Library.Common.Name,group=row_ID))+
  #geom_point(size=3)+geom_line(size=1.5)+
    #facet_wrap(~Component.Index,scales="free")+
    #theme(legend.position="bottom")
#Better, but 45 still looks bad as does -1 and 5 a little bit. Would be nicer to group by some clearer classification. export as 2500x1500 PNG and save as mixed.mod.significant.gut.subsection.features1

#now try the same plot with added errorbars
#ggplot(abund.longformat3[abund.longformat3$Library.Common.Name.x!="N/A",],aes(x=Gut_SubSection,y=mean_relabund,color=Library.Common.Name.x,group=row_ID))+
  #geom_point(size=3.5)+geom_line(size=2)+
  #geom_pointrange(aes(ymin=mean_relabund-SE,ymax=mean_relabund+SE))+
    #facet_wrap(~Component.Index.x,scales="free")+
    #theme(legend.position="bottom")

#now try the same plot with facet by ClassyFire.Parent.Level.1
ggplot(abund.longformat4,aes(x=Gut_SubSection.x,y=mean_relabund,color=Common.Name.Analog,group=row_ID))+
  geom_point(size=3.5)+geom_line(size=2)+
  geom_pointrange(aes(ymin=mean_relabund-SE,ymax=mean_relabund+SE))+
...
#export as 12X12 portrait PDF and save as mixed.mod.significant.gut.subsection.features6. Use for Fig 7

#now try the same plot using geom_smooth
ggplot(abund.longformat1[abund.longformat1$Library.Common.Name!="N/A",],aes(x=Gut_SubSection.y,y=abund,color=Library.Common.Name,group=row_ID))+
  geom_point()+geom_smooth()+
    facet_wrap(~Component.Index,scales="free")+
    theme(legend.position="bottom")
#Better, but 45 still looks bad as does -1 and 5 a little bit. Would be nicer to group by some clearer classification. 2500x1000 PNG and save as mixed.mod.significant.gut.subsection.features4

```

I think the points at means with SE pointranges look best, although SE pointrange might need to be adjusted to reflect non normal distribution.

Next, plot individual features Emily suggested.
