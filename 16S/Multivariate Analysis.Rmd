...
```{r}
#Model F4-F8 distance matrix without PC samples, variables of interest: Species, Gut_subsection, and Fish_Number. Use adonis2 marginal tests, first testing the effect of the interaction 
perm=how(nperm=999)
setBlocks(perm)=with(metadata.fish.F4.F8.unifrac,Species)
permanova.gut.subsection=adonis2(weighted.unifrac.F4.F8.dist~Gut_SubSection+Fish_Number,by="margin",data=metadata.fish.F4.F8.unifrac,permutations=perm)
permanova.gut.subsection
#Marginally significant interaction.

permanova.gut.subsection1=adonis2(weighted.unifrac.F4.F8.dist~Gut_SubSection+Species,by="margin",data=metadata.fish.F4.F8.unifrac,permutations=perm)
permanova.gut.subsection1
...
```{r}
colvec2=c("mediumpurple1",viridis(n=7,option="D",begin=.15)) #generate colvec
...
ordiellipse(nmds.weighted.unifrac.F4.F8,metadata.fish.F4.F8.unifrac$Gut_Zone,kind="sd",draw="polygon",lwd=2.5,border=c("mediumpurple1",viridis(n=3,option="D")[2:3]),col=c("mediumpurple1",viridis(n=3,option="D")[2:3])) #add SD ellipses, colored by Gut_Section. 
...
plot_grid(sobs.gut.subsection.plot,shannoneven.gut.subsection.plot,shannon.gut.subsection.plot) ##use the plot_grid function to plot the above 2 plots (par(mfrow=())) doesnt work with ggplot2). Save as PNG (995 x 750) and use as Figure 2 in manuscript.
```
...
#convert dispersion.gut.section1 into df of means and SE so it can be graphed as a barplot. Use as Table SX1 in manuscript
