...
NOTE, NEED TO UPDATE ALL ANALYSIS FOR NEW GUT_SECTION
...
metadata$Gut_SubSection=factor(metadata$Gut_SubSection,levels=c("ST","GI_1","GI_2","GI_3","GI_4","HG","HG_1","HG_2","HG_3","No_Inoc","","PC")) #reorder the levels of the Gut_SubSection factor. This will be useful later when generating ordinations.
...
test.no.strata=permanova.gut.section=adonis.II(weighted.unifrac.dist~Species*Gut_Zone,by="margin",data=metadata.fish.1,permutations=999)
...
permanova.gut.section.2=adonis.II(weighted.unifrac.2.dist~Species*Gut_Zone,by="margin",strata=metadata.fish.2.unifrac$Fish_Number,data=metadata.fish.2.unifrac,permutations=999)
permanova.gut.section.2 #Gut_Section, Species, and their interaction are significant.
...
#Next run pairwise adonis and the 3rd and 4th models to identify which gut regions differ from each other.
pairwise.gut.section.2=pairwise.adonis(weighted.unifrac.2.dist,metadata.fish.2.unifrac$Gut_Section)
pairwise.gut.section.2

pairwise.gut.subsection.3=pairwise.adonis(weighted.unifrac.F4.F8.2.dist,metadata.fish.F4.F8.2.unifrac$Gut_SubSection)
pairwise.gut.subsection.3

```

Both Gut Section/SubSection and Species have a significant effect on fish gut microbiome community structure, with no significant interaction, although there is a marinally significant interaction (p<.01) in the gut_subsection model. When Species and Gut_Section are the predictors, Gut_Section has a stronger effect on the fish gut microbiome then species. When Species and Gut_SubSection are the predictors, they have similar strength effects on the fish gut microbiome (Species has a slightly higher F value). Pairwise comparisons reveal that all Gut Sections are significantly different from each other (after padjustment) and that no gut subsections are significantly different from each other (after p adjustment) although ST is marginally significantly different from GI_4, HG_1, and HG_2.
...
colvec=viridis(n=3,option="D") #generate a vector of colors to correspond to Gut_Zone.
...
nmds.scores.2$Gut_Section=metadata.fish.F4.F8.2.unifrac$Gut_Section
...
Next, plot a NMDS in base R colored by Gut_SubSection but with ellipses by Gut_Section

```{r}
colvec2=viridis(n=8,option="D")
colvec3=viridis(n=4,option="D")
plot(nmds.weighted.unifrac.F4.F8.2,type="n") #Set up the NMDS plot environment
...
ordiellipse(nmds.weighted.unifrac.F4.F8.2,metadata.fish.F4.F8.2.unifrac$Gut_Section,kind="sd",draw="polygon",alpha=c(0,0),lwd=2.5,border=colvec3) #add SD ellipses, colored by Gut_Section. Export as 8.5x11 PDF
```
...
```{r}
#UPDATE FOR GUT_SECTION AND FIX MODELS
...
summary(adiv.mod.gut.section.sobs) #Note that including the Fish_Number ranef yields a singularity error regardless of species
anova(adiv.mod.gut.section.sobs,ddf="Kenward-Roger") #Gut_Section is significant
lsmeans(adiv.mod.gut.section.sobs, pairwise ~ Gut_Section, adjust="tukey") #HG and ST are not significantly different from each other, both are significantly different from GI.
...
lsmeans(adiv.mod.gut.section.shannoneven, pairwise ~ Gut_Section, adjust="tukey")
...
adiv.mod.gut.subsection.sobs=lmer(sobs~Gut_SubSection+Species+(1|Fish_Number),data=alpha.diversity.fish.F4.F8.2) #interaction term has been removed to prevent errors.
...
adiv.mod.gut.subsection.shannoneven=lmer(shannoneven~Gut_SubSection+Species+(1|Fish_Number),data=alpha.diversity.fish.F4.F8.2)
...
sobs.gut.subsection.plot=ggplot(alpha.diversity.2.mean.gut_subsection,aes(x=Gut_SubSection,y=sobs,fill=Gut_SubSection))+
  ...
  theme_classic()+
  theme(legend.position = "none")+
  geom_text(x=1:8,y=c(655,275,240,275,440,625,595,570),aes(label=c("A/C","B","B","B","B/D","C/D","C/D","C/D")))
...
dispersion.gut.section=betadisper(weighted.unifrac.F4.F8.2.dist,type="centroid",group=metadata.fish.F4.F8.2.unifrac$Gut_Section)
...
dispersion.gut.section1$Gut_Section=metadata.fish.F4.F8.2.unifrac$Gut_Section
dispersion.gut.section1$Gut_SubSection=metadata.fish.F4.F8.2.unifrac$Gut_SubSection
dispersion.gut.section1$Species=metadata.fish.F4.F8.2.unifrac$Species
dispersion.gut.section1$Fish_Number=metadata.fish.F4.F8.2.unifrac$Fish_Number

#convert dispersion.gut.section2 into df of means and SE so it can be graphed as a barplot.
dispersion.gut.section3=as.data.frame(aggregate(dispersion.gut.section1$distances,by=list(dispersion.gut.section1$Gut_Section),FUN=mean))
dispersion.gut.section3$se=aggregate(dispersion.gut.section1$distances,by=list(dispersion.gut.section1$Gut_Section),FUN=stderror)[[2]]
colnames(dispersion.gut.section3)=c("Gut_Section","distance_to_centroid","se")
#do the same for gut subsection
dispersion.gut.subsection4=as.data.frame(aggregate(dispersion.gut.section1$distances,by=list(dispersion.gut.section1$Gut_SubSection),FUN=mean))
dispersion.gut.subsection4$se=aggregate(dispersion.gut.section1$distances,by=list(dispersion.gut.section1$Gut_SubSection),FUN=stderror)[[2]]
...
anova(dispersion.mod.gut.section,ddf="Kenward-Roger") #Gut_Section is significant. species is marginally significant. Note that the interaction was not included in this model.
lsmeans(dispersion.mod.gut.section, pairwise ~ Gut_Section, adjust="tukey")
#ST and HG are significantly different from GI but not from each other.

#Model the sqrt distance values in response to Species, Gut_SubSection, and Fish_Number as a random effect.
dispersion.mod.gut.subsection=lmer(sqrt.distances~Species+Gut_SubSection+(1|Fish_Number),data=dispersion.gut.section2) #Warning "fixed-effect model matrix is rank deficient so dropping 2 columns / coefficients" This error no longer appears once the interaction term was removed.
...
dispersion.gut.subsection.plot.v1=ggplot(dispersion.gut.subsection4,aes(y=distance_to_centroid,x=Gut_SubSection,fill=Gut_SubSection))+
  geom_bar(stat="identity")+
  scale_fill_manual(values=viridis(n=8,option="D"))+
  theme_classic()+
  geom_errorbar(aes(ymin=distance_to_centroid-se,ymax=distance_to_centroid+se),width=.5)+
  geom_text(x=1:8,y=c(.35,.45,.4,.5,.5,.325,.35,.35),aes(label=c("B/C","B/D","B/C","B","B","C","C/D","C")))+
  ylim(0,.5)
...
#visualize dispersion as a function of gut_section
dispersion.gut.section.plot.v1=ggplot(dispersion.gut.section3,aes(y=distance_to_centroid,x=Gut_Section,fill=Gut_Section))+
  geom_bar(stat="identity")+
  scale_fill_manual(values=viridis(n=3,option="D"))+
  theme_classic()+
  geom_errorbar(aes(ymin=distance_to_centroid-se,ymax=distance_to_centroid+se),width=.5)+
  geom_text(x=1:3,y=c(.37,.45,.33),aes(label=c("A","B","A")))+
  ylim(0,.45)

dispersion.gut.section.plot.v1
```
